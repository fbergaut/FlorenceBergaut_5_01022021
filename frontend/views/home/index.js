  ...
  (allProducts) => {
    allProducts.map((bear) => {
      ...
      teddies += `<div class="col-12 col-lg-3">
                      <a href="../products/product.html?id=${bear._id}&price=${bear.price / 100}" class="productsLink">
                          <div class="card border-0 bearProduct">
                            <img class="card-img-top" src="${bear.imageUrl}" alt="${bear.name}">
                          </div>
                          <div class="card-body bg-white">
                            <h5 class="card-title text-center">${bear.name}</h5>
                            <p class="card-text text-center">${bear.price / 100} €</p>
                          </div>
                      </a>
                    </div>`;
    });
